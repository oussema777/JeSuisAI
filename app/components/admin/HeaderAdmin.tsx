...
import { Bell, Search, ChevronDown, X, Check, Trash2, User, LayoutDashboard, LogOut, Languages } from 'lucide-react';
...
export function HeaderAdmin({ pageTitle, breadcrumb }: HeaderAdminProps) {
  ...
  const getRelativeTime = (dateString: string) => {
    ...
    if (diffInSeconds < 3600) return t('notifications.time.min', { count: Math.floor(diffInSeconds / 60) });
    if (diffInSeconds < 86400) return t('notifications.time.hour', { count: Math.floor(diffInSeconds / 3600) });
    if (diffInSeconds < 604800) return t('notifications.time.day', { count: Math.floor(diffInSeconds / 86400) });
    return date.toLocaleDateString(locale === 'fr' ? 'fr-FR' : 'en-US', { day: 'numeric', month: 'short' });
  };

  // Add this function after getRelativeTime and before getNotificationAction
  ...
  return (
    <header className="h-16 lg:h-[72px] bg-white border-b border-neutral-200 shadow-sm fixed top-0 left-0 lg:left-[260px] right-0 z-40">
      <div className="h-full px-4 lg:px-10 flex items-center justify-between">
        ...
        <div>
          {breadcrumb && (
            <div className="flex items-center gap-2 mb-1 hidden lg:flex">
        ...
        <div className="flex items-center gap-3 lg:gap-4">
          ...
          <div className="flex items-center bg-neutral-100 rounded-lg p-1 mr-2 shadow-inner">
            ...
            <button
              ...
              title="English"
            >
              <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 60 30" className="w-6 h-4 rounded-sm"><clipPath id="a1"><rect width="60" height="30"/></clipPath><g clipPath="url(#a1)"><rect width="60" height="30" fill="#012169"/><path d="M0 0l60 30M60 0L0 30" stroke="#fff" strokeWidth="6"/><path d="M0 0l60 30M60 0L0 30" stroke="#C8102E" strokeWidth="4" clipPath="url(#a1)"/><path d="M30 0v30M0 15h60" stroke="#fff" strokeWidth="10"/><path d="M30 0v30M0 15h60" stroke="#C8102E" strokeWidth="6"/></g></svg>
            </button>
          </div>
                ...
                {notifications.length > 0 && (
                  <div className="px-4 py-3 border-t border-neutral-200 bg-neutral-50">
                    <Link
                      ...
                      className="text-primary hover:text-primary/80 text-sm font-medium text-center block"
            ...
            {showUserMenu && (
              <div className="absolute right-0 mt-2 w-56 bg-white rounded-xl shadow-2xl border border-neutral-200 z-50 py-2 animate-in fade-in zoom-in-95 duration-200">
                ...
                <Link href="/" className="flex items-center gap-3 px-4 py-2.5 text-neutral-700 hover:bg-neutral-50 transition-colors" onClick={() => setShowUserMenu(false)}>
                  ...
                  <span style={{ fontSize: '14px', fontWeight: 500 }}>{t('user_menu.view_site')}</span>
                </Link>
                
                <Link href="/admin/parametres" className="flex items-center gap-3 px-4 py-2.5 text-neutral-700 hover:bg-neutral-50 transition-colors" onClick={() => setShowUserMenu(false)}>
                  <User className="w-4.5 h-4.5 text-primary" />
                  <span style={{ fontSize: '14px', fontWeight: 500 }}>{t('user_menu.settings')}</span>
                </Link>
                ...
                >
                  ...
                  <span style={{ fontSize: '14px', fontWeight: 500 }}>{t('user_menu.logout')}</span>
                </button>
              </div>
            )}
          </div>
