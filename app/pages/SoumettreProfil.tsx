...
import { ChevronLeft, Upload, X, FileText, User, Send, Loader2 } from 'lucide-react';
...
// Contribution proposée options
const CONTRIBUTIONS_FR = [
  'Investissement',
  'Epargne',
  'Compétences',
  'Dons',
...
const CONTRIBUTIONS_EN = [
  'Investment',
  'Savings',
  'Skills',
  'Donations',
  'Networks & influence',
  'Fair trade & solidarity tourism',
];
...
const PAYS_EN = [
  ...
  'Saudi Arabia', 'Argentina', 'Australia', 'Austria', 'Belgium', 'Brazil',
  'Burkina Faso', 'Burundi', 'Cameroon', 'Canada', 'Central African Republic', 'China',
  'Congo (Brazzaville)', 'Congo (Kinshasa)', 'Ivory Coast', 'Denmark',
  'Egypt', 'United Arab Emirates', 'Spain', 'United States', 'Ethiopia', 'Finland',
  'France', 'Gabon', 'Ghana', 'Greece', 'Guinea', 'India', 'Italy', 'Japan',
  'Kenya', 'Liberia', 'Mali', 'Morocco', 'Mexico', 'Nigeria', 'Norway',
  'New Zealand', 'Netherlands', 'Poland', 'Portugal', 'United Kingdom',
  'Russia', 'Senegal', 'Sweden', 'Switzerland', 'Chad', 'Togo', 'Tunisia', 'Turkey',
];

export default function SoumettreProfil() {
  ...
  const CONTRIBUTIONS = currentLocale === 'en' ? CONTRIBUTIONS_EN : CONTRIBUTIONS_FR;
  const PAYS = currentLocale === 'en' ? PAYS_EN : PAYS_FR;
  ...
  const handleContributionToggle = (contribution: string) => {
    setFormData(prev => ({
      ...prev,
      contributionsProposees: prev.contributionsProposees.includes(contribution)
        ? prev.contributionsProposees.filter(c => c !== contribution)
        : [...prev.contributionsProposees, contribution],
    }));
  };
  ...
  const validateForm = () => {
    ...
    const othersLabel = currentLocale === 'en' ? 'Others (specify)' : 'Autres (préciser)';
    if (formData.domainesAction.includes(othersLabel) && !formData.autreDomaineAction.trim()) return false;
    if (formData.contributionsProposees.length === 0) return false;
    if (!formData.toutesLesVilles && formData.villesSelectionnees.length === 0) return false;
    if (!formData.email.trim()) return false;
    if (formData.autoriserPublication === null) return false; // REQUIRED: Authorization must be selected
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (const file of formData.documentsJoints) {
        ...
        body.append('bucket', 'fichiers-profils');
        body.append('folder', 'profils');
      ...
      const dataToInsert = {
        ...
        contributions_proposees: formData.contributionsProposees,
        ...
        autorisation_publication: formData.autoriserPublication === true ? 'oui' : 'non',
      };
      ...
      if (insertError) {
        ...
        toast.error('Erreur technique lors de la soumission');
        setIsSubmitting(false);
        return;
      }
      ...
      try {
        const targetVilles = formData.toutesLesVilles
          ? [] // For "toutes", we skip ville-specific annonceur emails
      ...
      try {
        await notificationService.notifyAdmins({
          ...
          message: `${formData.prenom} ${formData.nom} a soumis son profil de contribution.`,
          data: {
      ...
      router.push(
        `/succes-soumission?type=profil&userName=${encodeURIComponent(formData.prenom + ' ' + formData.nom)}&userEmail=${encodeURIComponent(formData.email)}`
      );
    } catch (err) {
          ...
          <div className="flex items-start gap-4 mb-6">
            ...
            <div>
              <h2 className="text-white mb-2" style={{ fontSize: '32px', lineHeight: '1.2', fontWeight: 600 }}>
                {currentLocale === 'en' ? 'Describe your contribution for Cameroon here!' : 'Décrivez-nous ici votre contribution pour le Cameroun !'}
              </h2>
              <p className="text-white/90" style={{ fontSize: '18px', lineHeight: '1.6', fontWeight: 400 }}>
                {currentLocale === 'en' 
                  ? 'The diaspora focal point of the targeted city(ies) will be directly informed and will be able to study your proposal…' 
                  : 'Le point focal diaspora de la ou les villes ciblées sera directement informé et pourra étudier votre proposition…'}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="max-w-4xl mx-auto px-5 md:px-10">
          <form onSubmit={handleSubmit}>
            <div className="bg-white rounded-xl shadow-sm border border-neutral-200 p-8 md:p-10">
              ...
              {/* SECTION 2: CONTRIBUTION PROPOSÉE */}
              <div className="mb-12">
                <label className="block text-neutral-700 mb-4" style={{ fontSize: '16px', fontWeight: 600 }}>
                  {currentLocale === 'en' ? 'Proposed contribution' : "Contribution proposée"} <span className="text-accent-red">*</span>
                </label>
                
                <div className="grid grid-cols-1 md:grid-cols-2 gap-3 p-4 border border-neutral-300 rounded-lg">
                  {CONTRIBUTIONS.map((contribution) => (
                    <label key={contribution} className="flex items-start gap-3 cursor-pointer group">
                      <input
                        type="checkbox"
                        checked={formData.contributionsProposees.includes(contribution)}
                        onChange={() => handleContributionToggle(contribution)}
                        className="mt-1 w-4 h-4 text-primary rounded"
                        disabled={isSubmitting}
                      />
                      <span className="text-neutral-700 group-hover:text-neutral-900" style={{ fontSize: '14px', fontWeight: 400 }}>
                ...
                <div className="space-y-6">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={currentLocale === 'en' ? 'Describe your profile and contribution in detail...' : "Décrivez votre profil et votre contribution en détail..."}
                      disabled={isSubmitting}
                    />
                  </div>
                </div>
              ...
              <div className="mb-8 p-4 bg-neutral-50 border border-neutral-200 rounded-lg">
                <label className="block mb-3 text-neutral-700" style={{ fontSize: '15px', fontWeight: 500 }}>
                  {currentLocale === 'en' 
                    ...
                    : 'Autorisez-nous à publier votre contribution sous forme de témoignage sur la plateforme ?'} <span className="text-accent-red">*</span>
                </label>
